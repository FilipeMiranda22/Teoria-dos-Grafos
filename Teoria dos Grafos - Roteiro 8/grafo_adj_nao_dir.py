...
class CargaInvalidaException(Exception):
    pass
...
class Grafo:
    ...
    def dijkstra(self, v, f, recargas, carga_atual, carga_maxima):
        ...
        if carga_atual > carga_maxima:
            raise CargaInvalidaException('A carga inicial não pode ser maior que a máxima.')
    ...
    def caminho_dijkstra(self, v, f, w, recargas, vertices, carga_maxima):
        ...
        if w in recargas:
            soma1 = carga_maxima - (vertices[w][3] - vertices[w][0])
            vertices[w][3] += soma1
    ...
    def peso(self, v1, v2):
        for c in range(len(self.N)):
            for i in range(len(self.N)):
                if (v1 == self.N[c] and v2 == self.N[i]) or (v1 == self.N[i] and v2 == self.N[c]):
                    if self.M[c][i] != '-':
                        return self.M[c][i]

    def vertice_min(self):
        aux = math.inf
        for c in range(len(self.N)):
            for i in range(len(self.N)):
                if self.M[c][i] != '-' and self.M[c][i] != 0 and self.M[c][i] < aux:
                    aux = self.M[c][i]
                    vmin = self.N[i]
        return vmin

    def prim(self):
        raiz = self.vertice_min()
        vertices = [raiz]
        arestas_selecionadas = []

        peso = 0

        r_vertices = list(self.N)
        r_vertices.remove(raiz)

        for i in range(len(r_vertices)):
            min_cost = math.inf
            va, vb = None, None
            for v1 in vertices:
                for v2 in r_vertices:
                    cost = self.peso(v1, v2)
                    if cost != 0:
                        if cost < min_cost:
                            va = v1
                            vb = v2
            ...
            if min_cost < math.inf:
                arestas_selecionadas.append((va, vb, min_cost))
                vertices.append(vb)
                r_vertices.remove(vb)
                peso += min_cost

        return arestas_selecionadas, peso
